...
`lib/azimuth.mjs` (`POST /api/map/azimuth`): rotation/reflection-invariant Rayleigh Z_m (m=2,3,4,6,12) + largest circular gap on the placement-plane angles, refit per null draw, Holm-corrected two-sided plus-one tails with per-seed reproducibility. Binary variant runs the fixed-margin null plus an atomicity control (distinct-pattern dedup, m=1 audit, null dedup-size support); continuous variant verifies caller vectors against the stored row fingerprints and runs a per-column shuffle null. `admitted:false` always: m=1 is a centered-plane radius/multiplicity reweighting, the binary channel lacks a size-matched de-atomized null, and the continuous shuffle null is over-strong for negative claims. `AzimuthBounds.lean` proves the D₄ gauge identities and replicate scaling in core Lean; CI compiles it and runs `node test-azimuth.mjs` (7 checks, real 301×24 fixture).
