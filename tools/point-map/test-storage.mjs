import {radiusProfile} from './lib/radius-diagnostics.mjs';
...
m.radius_profile=radiusProfile(m,{coordinate_epsilon:0.001,distance_error_bound:1e-12});
...
assert.equal(JSON.stringify(decoded.radius_profile),JSON.stringify(m.radius_profile));
...
console.log(JSON.stringify({checks:8,failed:0,unpacked_bytes:new TextEncoder().encode(JSON.stringify(m)).length,stored_bytes:new TextEncoder().encode(encoded).length,N:400,D:768,d:24,scope:'lossless diagnostic packing, unchanged frame and geometry'}));
