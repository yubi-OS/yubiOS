...
- Two separate corpora, fit independently.** SELF.md and SELF-CHANGELOG.md have different structure (rows vs entries) and different primitive bases. They are NOT a single corpus. Each gets its own Stage 1 fit + Stage 5 re-fit + per-corpus verification metric.
- Expanded corpus scope (v1.1).** When the corpus is expanded to include all 10 memory files (USER_PREFERENCES, COMPANY, RULES, SAUNA_IDENTITY, SAUNA_TOOLS, USER_PROFILE, USER_RELATIONSHIPS, RECENT_ACTIVITY) + PROJECT_RULES, a unified 9-D memory-file basis replaces the per-file bases (the audit-trail + substrate lenses merge into one coverage basis).
- Granularity is canonical: each version = one corpus item.** A SELF-CHANGELOG entry (v0.1, v0.2, ...) is one item. A SELF.md row (one strength, bias, anti-pattern, mode, energy, growth edge) is one item.
- The 20-item gate is the binding constraint.** SELF-CHANGELOG has 18 entries (v0.1 → v0.18); SELF.md has ~51 rows. The decomposition rule applies — but for memory files, decomposition splits each section into sub-events rather than splitting rows.

**In-repo touchpoints** — sections this skill owns or extends: Extended description, Philosophy, When to Use, The Model — 5-stage pipeline retargeted for self-docs.

**Boundary case** — when the request only names a trigger without the artifact it acts on, route to the owning surface instead of improvising here.
## Guidelines

1. p4 has_constraint  — section/entry has hard constraint or anti-pattern (Must, Never, Always, Don't, ban)
2. One-shot pushing without SELF-CHANGELOG entry** — the audit trail is the relationship, not a byproduct. Every push MUST be accompanied by a SELF-CHANGELOG entry per the discipline.
